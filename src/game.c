...
void display_game_over(void)
{
    ...
    BMP_Display("/home/wangjun/pics/gameover.bmp", ((SCREEN_WIDTH - 200) / 2), ((SCREEN_HEIGHT - 100) / 2));
    sleep(3);
    goto_home_page();
}
/*
    goto_home_page : 回到主界面
    返回值:
        无
*/
void goto_home_page(void)
{
    ...
    BMP_Display("/home/wangjun/pics/begingame.bmp", MATRIX_X0 + 50, 200);
    BMP_Display("/home/wangjun/pics/exitgame.bmp", MATRIX_X0 + 50, 300);
    struct point p;
    do
    {
        p = Get_point();
        printf("x = %d , y = %d \n", p.x, p.y);
        //退出游戏
        if(((p.x >= MATRIX_X0 +25 && p.x <= MATRIX_X0 + 300) && (p.y >= 400 && p.y <= 480)))
        {
            pro_Init();
        }
    } while (!((p.x >= MATRIX_X0 +25 && p.x <= MATRIX_X0 + 300) && (p.y >= 200 && p.y <= 300)));


    start_new_game();
}
/*
    start_new_game: 开始新游戏,初始化游戏状态
*/
void start_new_game()
{
    // 将原来矩阵中的数字清零
    memset(matrix_2048_1, 0, sizeof(matrix_2048_1));
...
void place_random_number()
{
    ...
    if (empty_count > 0)
    {
        ...
        int new_value = (rand() % 10 == 0) ? 4 : 2; // 随机生成2或4 90% 概率生成2 ,10%概率生成4
...
{
    char pathname[32];
    sprintf(pathname, "/home/wangjun/pics/empty.bmp");
...
{
    char pathname[32];
    sprintf(pathname, "/home/wangjun/pics/%d_n.bmp", matrix_2048_1[row][col]);
